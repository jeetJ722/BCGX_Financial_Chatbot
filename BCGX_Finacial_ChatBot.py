...
final_report = pd.read_csv('Final_Report_BCGX.csv')
summary_report = pd.read_csv('final_data_report_BCGX.csv')
...
def financial_chatbot():
    ...
    if user_query ==  "What is the total revenue?":
        ...
        cash_ops = final_report[(final_report['Year'] == fiscal_year) & (final_report['Company'] == company_input)][
        ...
        assets_growth = \
        final_report[(final_report['Year'] == fiscal_year) & (final_report['Company'] == company_input)][
            'Assets Growth (%)'].values[0].round(4)
        ...
        liabilities_growth = \
        final_report[(final_report['Year'] == fiscal_year) & (final_report['Company'] == company_input)][
            'Liabilities Growth (%)'].values[0].round(4)
        ...
        cash_ops_growth = \
        final_report[(final_report['Year'] == fiscal_year) & (final_report['Company'] == company_input)][
            'Cash Flow from Operations Growth(%)'].values[0].round(4)
